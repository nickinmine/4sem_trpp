...
<body>
	...
	<?php
		...
		if (!$cred)
			echo "<p>Информация по кредиту №$id не найдена!</p>\n";
		else {
			$z = NULL; // задолженность
			$res = credit_tail_sum($id, $z, $mysqli);
			verify($res == "", $res);

			echo "<p>Информация по кредиту №" . $cred["id"] . " от " . out_date($cred["opendate"]) . " по состоянию на " . out_date($curdate) . "</p>\n";
			echo "<p>\n";
...
<?php if ($cred) { ?>
	<table border="1">
		<caption>График погашений</caption>
		<tr>
			<th>Дата платежа</th>
